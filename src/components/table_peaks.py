...
def render(app: Dash, data: pd.DataFrame) -> html.Div():
    ...
    def update_table(prominence, intervall):
    ...
    df = update_table(prominence=500, 
                      intervall=[data["R.Time (min)"].min(),
                      data["R.Time (min)"].max()],
                      )
    ...
    fig = dash_table.DataTable(
        id=ids.PEAK_TABLE,
        data=df,
        columns=[{'name': i, 'id': i} for i in ["Retention Time [min]", "Intensity [AU × 10⁻³]", "Peak Type"]],
        page_size=10,
